...
The {repository-release-latest}[`latest`] release containing the <<default,
`default`>> <<packages, package>>, the link:Gemfile.lock[] and link:gemset.nix[]
files, and the link:flake.lock[] file are automatically updated by the
link:.github/workflows/deploy.yml[], link:.github/workflows/bundix_lock.yml[],
and link:.github/workflows/nix_flake_update.yml[] GitHub actions respectively.
...
a| Compiles the <<docbook, `docbook`>>, <<html, `html`>>, <<manpage,
   `manpage`>>, and <<pdf, `pdf`>> <<packages, packages>>.
